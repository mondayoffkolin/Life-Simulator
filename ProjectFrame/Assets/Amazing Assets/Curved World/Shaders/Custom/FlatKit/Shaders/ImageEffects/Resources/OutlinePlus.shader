		...
		Pass
		{
			...
			fixed4 frag(v2f i): SV_Target
			{
				...
				float2 uv3 = i.uv + float2(left, bottom);
					...
					float d0 = Linear01Depth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, uv0));
					float d1 = Linear01Depth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, uv1));
					float d2 = Linear01Depth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, uv2));
					float d3 = Linear01Depth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, uv3));
