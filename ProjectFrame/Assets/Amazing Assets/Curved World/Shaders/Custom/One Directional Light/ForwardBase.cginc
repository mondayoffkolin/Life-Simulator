...
float _IncludeVertexColor;
	...
	float2 _SecondaryTex_Scroll;
		...
		float _SecondaryNormalMap_UV_Scale;
	#endif
#endif
...
{ 
	...
	o.texcoord.xy += _MainTex_Scroll.xy * _Time.x;


	#if defined(_TEXTUREMIX_BY_MAIN_ALPHA) || defined(_TEXTUREMIX_BY_SECONDARY_ALPHA) || defined(_TEXTUREMIX_MULTIPLE) || defined(_TEXTUREMIX_ADDITIVE) || defined(_TEXTUREMIX_BY_VERTEX_ALPHA)
		o.texcoord.zw = v.texcoord.xy * _SecondaryTex_ST.xy + _SecondaryTex_ST.zw;
		o.texcoord.zw += _SecondaryTex_Scroll.xy * _Time.x;
	#endif
...
float4 frag (vOutput i) : SV_Target
{
	float4 mainTex = tex2D(_MainTex, i.texcoord.xy);
		...
		float4 decal = tex2D(_SecondaryTex, i.texcoord.zw);
		...
		retColor *= tex2D(_SecondaryTex, i.texcoord.zw);
	#elif defined(_TEXTUREMIX_ADDITIVE)
		float4 retColor = mainTex;
		retColor = saturate(retColor + tex2D(_SecondaryTex, i.texcoord.zw));
	#elif defined(_TEXTUREMIX_BY_VERTEX_ALPHA)
		float vBlend = clamp(_SecondaryTex_Blend + i.color.a, 0, 1);
		float4 retColor = lerp(mainTex, tex2D(_SecondaryTex, i.texcoord.zw), vBlend);
	#else
		float4 retColor = mainTex;
	#endif
	...
	//Vertex Color
	retColor *= lerp(float4(1, 1, 1, 1), i.color, _IncludeVertexColor);
	...
	#ifdef _NORMALMAP
		float4 normalMap = tex2D(_NormalMap, i.texcoord.xy * _NormalMap_UV_Scale);				

		#ifdef _TEXTUREMIX_BY_MAIN_ALPHA
			float4 secondN =  tex2D(_SecondaryNormalMap, i.texcoord.zw *_SecondaryNormalMap_UV_Scale);
	...
	#ifdef _EMISSION
		retColor.rgb += tex2D(_EmissionMap, i.texcoord.xy * _EmissionMap_UV_Scale).rgb * _EmissionColor.rgb;
	#endif
